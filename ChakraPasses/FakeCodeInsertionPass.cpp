...
static void populateAndTerminateBlockWithJunk(BasicBlock *block,
  ...
  // CREATE A VOLATILE SIDE EFFECT
  // If we generated any instructions, store the result of the last one
  // into a dummy stack variable. The 'true' flag makes the store volatile.
  if (lastVal) {
...
static bool addFakeCodeToFunction(Function &F) {
  ...
  // Create a dummy stack variable at the start of the function
  ...
  for (unsigned i = 0; i < numBlocksToInsert; ++i) {
    ...
    // Pass the dummy variable to the population function.
